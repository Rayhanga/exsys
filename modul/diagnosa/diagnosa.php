<title>Diagnosa - DepExSys</title>
<?php
switch ($_GET['act']) {

  default:
    if ($_POST['submit']) {
      $arcolor = array('#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff');
      date_default_timezone_set("Asia/Jakarta");
      $inptanggal = date('Y-m-d H:i:s');

      $arbobot = array('0', '1', '0.8', '0.6', '0.4', '0.2', '0', '-0.4');
      $argejala = array();
...
// --------------------- END -------------------------
	...
	<h2 class='text text-primary'>Hasil Diagnosis &nbsp;&nbsp;<button id='print' onClick='window.print();' data-toggle='tooltip' data-placement='right' title='Klik tombol ini untuk mencetak hasil diagnosa'><i class='fa fa-print'></i> Cetak</button> </h2>
      ...
      echo "<div class='callout callout-default'>Jenis penyakit yang diderita adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round($vlpkt[1], 2)*100 . " % (" . $vlpkt[1] . ")<br></h3>";
	 ...
	 <div class='alert alert-success alert-dismissible'>
      ...
      while ($r3 = mysqli_fetch_array($sql3)) {
        ...
        echo '<td class="opsi"><select name="kondisi[]" id="sl' . $i . '" class="opsikondisi"/><option data-id="0" value="0">Pilih yang sesuai</option>';
        ...
        <script type="text/javascript">
          $(document).ready(function () {
            var arcolor = new Array('#979797', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000');
            setColor();
            ...
            function setColor()
            {
              ...
              $('tr td select#sl<?php echo $i; ?>.opsikondisi').css('color', color);
              console.log(color);
            }
          });
        </script>
